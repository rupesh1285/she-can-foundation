    ...
    >
      <div className="flex items-center justify-between">
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link 
                key={link.name} 
                to={link.href} 
                className={`text-sm font-bold tracking-wide transition-colors relative group ${isActive ? 'text-[#E11D48]' : 'text-gray-700 hover:text-[#E11D48]'}`}
              >
                {link.name}
                <span className={`absolute -bottom-1 left-0 h-0.5 bg-[#E11D48] transition-all duration-300 rounded-full ${isActive ? 'w-full' : 'w-0 group-hover:w-full'}`}></span>
              </Link>
            )
          })}
        </div>
        ...
        <div className="hidden lg:flex items-center gap-4">
          <Link 
            to="/donate" 
            className="px-6 py-2.5 rounded-full bg-gradient-to-r from-[#E11D48] to-[#FDA4AF] text-white text-sm font-bold shadow-[0_10px_20px_-10px_rgba(225,29,72,0.6)] hover:shadow-[0_15px_30px_-10px_rgba(225,29,72,0.8)] hover:-translate-y-0.5 transition-all duration-300"
          >
            Donate
          </Link>
        </div>
            ...
            <div className="flex flex-col gap-4 pt-6 pb-4 px-2">
              {navLinks.map((link) => (
                <Link 
                  key={link.name} 
                  to={link.href} 
                  className={`text-lg font-bold ${location.pathname === link.href ? 'text-[#E11D48]' : 'text-[#0F172A]'}`}
                >
                  {link.name}
                </Link>
              ))}
              <div className="h-px bg-gray-200/50 w-full my-2"></div>
              <Link to="/donate" className="mt-2 text-center py-4 rounded-xl bg-gradient-to-r from-[#E11D48] to-[#FDA4AF] text-white font-bold">Donate Now</Link>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
